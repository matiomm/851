<header class="header">
    <a href="index.php"><img src="img/icono.png" class="header-icono"></a>
    <div class="header mano">
        <div class="actualizado-desktop">
            <h4 class="header_h4">PROBABILIDADES
                <span style="color:#da1f2b">ACTUALIZADAS</span>
            </h4>
            <h3 class="header_h3">03
                <span style="color:#da1f2b">/</span>07
                <span style="color:#da1f2b">/</span>19
                <span style="color:#da1f2b">15:26</span> hrs.</h3>
        </div>
        <div class="actualizado-movil" style="display:none">
            ACTUALIZADO
            <span color=#da1f2b>03/07/19</span> 15:26
        </div>
        <div class="titulo">
            <h1 class="header_h1">OCHO
                <span style="color:#da1f2b">CINCO</span>UNO</h1>
            <h3 class="header_h3">PREDICCIONES
                <span style="color:#da1f2b">EN</span>TIEMPO
                <span style="color:#da1f2b">REAL</span>
            </h3>
        </div>
    </div>
    <div class="cont logo-nav-cont">
        <nav class="navigation">
            <ul>
                <li>
                    <a href="index.php">Predicciones</a>
                </li>
                <li>
                    <a href="modelos.php">Modelos</a>
                </li>
                <li>
                    <a href="nosotros.php">Nosotros</a>
                </li>
                <li>
                    <a href="contacto.php">Contacto</a>
                </li>
                <li>
                    <a href="resultadosconcurso.php">Concurso</a>
                </li>
            </ul>
        </nav>
    </div>
</header>
